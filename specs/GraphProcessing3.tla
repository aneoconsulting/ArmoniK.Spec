...
(*****************************************************************************)
(* This module extends the 'GraphProcessing2' specification with the user-    *)
(* driven *pausing* and *stopping* (cancellation) of tasks, mirroring the     *)
(* refinement that 'TaskProcessing3' performs over 'TaskProcessing2'.          *)
(*                                                                           *)
(* Two control sets are introduced:                                          *)
(*   - stoppingRequested: tasks whose cancellation has been requested;        *)
(*   - pausingRequested:  tasks whose pausing has been requested.             *)
(*                                                                           *)
(* and two new lifecycle states (carried by TP3State):                        *)
(*   - TASK_STOPPED: a task whose execution was cancelled before completion;  *)
(*   - TASK_PAUSED:  a task whose execution is temporarily postponed.         *)
(*                                                                           *)
(* The graph machinery (data-dependency graph, object lifecycle, retries) is  *)
(* inherited unchanged from 'GraphProcessing2'. Pausing and stopping are      *)
(* layered on top as task-lifecycle operations: they never mutate the         *)
(* dependency graph nor the object states. A paused task is parked out of the *)
(* staged/assigned pool and can be resumed; a stopped task is parked          *)
(* permanently (it never produces its output objects and is treated as a      *)
(* non-viable node for object derivation).                                    *)
(*                                                                           *)
(* Because stopping is a deliberate user action, the user owes a liveness     *)
(* counterpart: if an object that stays a target forever loses its last       *)
(* production path to stopping (becomes underivable), a new path is           *)
(* eventually submitted -- otherwise the user simply untargets the object.    *)
(* This is captured by the StrandedTargetEventuallyRederivable assumption,    *)
(* the stopping-era analogue of OpenUpstreamEventuallyClosed.                 *)
(*                                                                           *)
(* The module defines two refinement mappings:                                *)
(*   - RefineTaskProcessing3: projects the graph state away, exhibiting the    *)
(*     task sublanguage as a behaviour of 'TaskProcessing3' (analogue of the  *)
(*     'GraphProcessing1' -> 'TaskProcessing1' mapping);                       *)
(*   - RefineGraphProcessing2: collapses TASK_STOPPED to TASK_DISCARDED (a     *)
(*     cancelled task will never produce its outputs, like a discarded one)    *)
(*     and TASK_PAUSED to TASK_STAGED, exhibiting the system as a behaviour    *)
(*     of 'GraphProcessing2' (analogue of the 'TaskProcessing3' ->            *)
(*     'TaskProcessing2' mapping).                                            *)
(*****************************************************************************)
 ...
 * The cancellation of a set 'T' of known tasks is requested.
 *)
...
(**
 * FAIRNESS CONDITIONS
 * The graph/retry fairness of GraphProcessing2, augmented with weak fairness
 * for the acknowledgment of stopping, pausing and resuming requests:
 *   - a task whose stopping is acknowledgeable is eventually stopped;
 *   - a task whose pausing is acknowledgeable is eventually paused;
...
Fairness ==
    ...
    /\ \A t \in Task:
        ...
        \* /\ WF_vars(\A o \in Object: o \in objectTargets /\ \E d \in GP3Derivation(o): t \in d.node /\ ResumeTasks({t}))
        \* /\ WF_vars(\A o \in Object: o \in objectTargets /\ \E d \in GP3Derivation(o): t \in d.node /\ t \in StoppedTask /\ DiscardTasks({t}))
        \* Remove ResumeTasks fairness
        \* Add discarding of a stopped task

(**
 * LIVENESS CONSTRAINT
 * For every object that is currently a target, the open upstream eventually
 * becomes closed under additions. Inherited unchanged from GraphProcessing2.
 *)
...
(**
 * LIVENESS CONSTRAINT (new in GraphProcessing3)
 * Stopping is a deliberate user action: by cancelling a task the user signals
 * that production of its outputs should proceed along a different path -- an
 * already-existing alternative, or a new one the user undertakes to submit --
 * or that the result is no longer wanted (in which case the user untargets
 * it). This is the environment counterpart of stopping: the system cannot, on
 * its own, re-create a cancelled production path, so it is assumed of the user.
 *
 * Formally: for every object that REMAINS a target forever, becoming
 * underivable (no viable derivation survives -- e.g. its last production path
 * was stopped) is always eventually followed by becoming derivable again (a
 * new path is submitted). The assumption is conditioned on `[](o is a target)`
 * precisely so that the obligation lifts as soon as the user untargets `o`:
 * untargeting is the alternative escape hatch the user always retains.
 *
 * This is the minimal assumption needed for the liveness of targeted objects
 * under stopping: it demands only re-derivability (re-submission of a path),
 * not completion -- the existing fairness then drives that path to produce the
 * object, exactly as for any other target.
 *)
StrandedTargetEventuallyRederivable ==
    \A o \in Object :
        [](o \in objectTargets)
            => (GP3Derivation(o) = {} ~> GP3Derivation(o) /= {})
...
Spec ==
    ...
    /\ Fairness
    /\ OpenUpstreamEventuallyClosed
    /\ \A t \in Task: <>[][~PauseTasks({t})]_vars
...
(**
 * REFINEMENT
 * Projecting the graph, object and stop/pause state away exhibits the task
 * sublanguage of GraphProcessing3 as a behaviour of TaskProcessing3. This is
 * the analogue of the GraphProcessing1 -> TaskProcessing1 refinement.
 *)
...
(**
 * REFINEMENT
 * A stopped task is one that will never produce its outputs (their production,
 * if still wanted, proceeds along another path); from GraphProcessing2's point
 * of view that is exactly a DISCARDED task. A paused task is parked out of the
 * staged/assigned pool and can be resumed, so it collapses to STAGED. Under
 * this mapping every GraphProcessing3 step is a GraphProcessing2 step (e.g.
 * StopTasks maps to DiscardTasks, ProcessTasks-to-STOPPED to the crash branch
 * of ProcessTasks, PauseTasks of an assigned task to ReleaseTasks).
 *)
taskStateBar ==
    [t \in Task |->
        CASE taskState[t] = TASK_STOPPED -> TASK_DISCARDED
          [] taskState[t] = TASK_PAUSED  -> TASK_STAGED
          [] OTHER                         -> taskState[t]
    ]

GP2 == INSTANCE GraphProcessing2 WITH taskState <- taskStateBar
