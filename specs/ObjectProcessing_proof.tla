...
<1>1. Init => DistinctObjectStates
    BY DEF Init, DistinctObjectStates, AreSetsDisjoint
<1>2. TypeInv /\ DistinctObjectStates /\ [Next]_vars => DistinctObjectStates'
    BY DEF TypeInv, DistinctObjectStates, AreSetsDisjoint, Next, vars,
...
<1>1. Fairness => Fairness!object(o)
    BY DEF Fairness
<1>. USE DEF OBJECT_UNKNOWN, OBJECT_REGISTERED, OBJECT_FINALIZED, UnknownObject,
             RegisteredObject, FinalizedObject
<1>2. []ObjectSafetyInv /\ [][Next]_vars /\ Fairness!object(o) /\ [](o \in objectTargets)
        => <>(o \in FinalizedObject)
    <2>1. ObjectSafetyInv /\ (o \in objectTargets) /\ ~(o \in FinalizedObject) => ENABLED <<o \in objectTargets /\ FinalizeObjects({o})>>_vars
        BY ExpandENABLED, TargetsAreKnown DEF TypeInv, FinalizeObjects, vars
