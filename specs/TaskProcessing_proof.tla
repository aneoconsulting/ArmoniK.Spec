...
<1>1. TypeInv /\ Init => AllocStateConsistent /\ ExclusiveAssignment
    BY DEF Init, TypeInv, AllocStateConsistent, ExclusiveAssignment, AssignedTask,
...
<1>2. TypeInv /\ AllocStateConsistent /\ ExclusiveAssignment /\ [Next]_vars
    ...
    <2>. SUFFICES ASSUME TypeInv, AllocStateConsistent, ExclusiveAssignment, [Next]_vars
                  PROVE AllocStateConsistent' /\ ExclusiveAssignment'
        OBVIOUS
    <2>. USE DEF TypeInv, AllocStateConsistent, ExclusiveAssignment, AssignedTask
...
<1>1. t \in AssignedTask
                    => \E a \in AgentId : {t} \subseteq agentTaskAlloc[a]
    BY DEF TaskSafetyInv, AllocStateConsistent
<1>. QED
    BY <1>1, ExpandENABLED DEF ProcessTasks, vars, TaskSafetyInv, TypeInv
