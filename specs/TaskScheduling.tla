...
(****************************************************************************)
EXTENDS FiniteSets, Graphs, GraphsExt, Naturals
 ...
 *)
CompleteObjects(S) ==
    /\ STS!IsStarted(AllPredecessors(deps, S))
    /\ SOP!Complete(S)
    /\ UNCHANGED << alloc, taskStatus, deps >>
...
SubmitTasks(G) ==
    ...
    IN /\ newDeps /= EmptyGraph
       ...
       /\ taskStatus' =
            [ t \in TaskId |->
                IF t \in G.node
                    THEN IF SOP!IsCompleted(Predecessors(newDeps, t))
                         THEN SUBMITTED
                         ELSE CREATED
                    ELSE taskStatus[t]
            ]
 ...
 *)
ScheduleTasks(a, S) ==
    /\ SOP!Lock(AllPredecessors(deps, S))
    /\ STS!Schedule(a, S)
    /\ UNCHANGED << deps >>
 ...
 *)
CompleteTasks(a, S) ==
    /\ SOP!IsCompleted(AllSuccessors(deps, S))
    /\ STS!Complete(a, S)
    /\ UNCHANGED << objectStatus, deps >>
...
ResolveTasks(S) ==
    /\ S /= {}
    /\ \A x \in AllPredecessors(deps, S): SOP!IsCompleted({x}) \/ SOP!IsLocked({x})
...
AllInputsLocked ==
    \A t \in TaskId :
        STS!IsStarted({t}) \/ STS!IsCompleted({t})
            => SOP!IsLocked(Predecessors(deps, t))
