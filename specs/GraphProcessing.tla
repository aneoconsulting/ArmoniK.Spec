...
IsConnectedDirectedGraph(G) == TRUE
...
(**
 * FAIRNESS CONDITIONS
 * Ensure that progress is eventually made for eligible tasks and objects.
 *)
Fairness ==
    ...
    \A o \in objectTargets:
        o \in FinalizedObject =>
            \E subDeps \in DirectedSubgraph(deps):
                ...
                /\ Leaves(subDeps) = {o}
                /\ IsConnectedDirectedGraph(subDeps)
                /\ TaskNode(subDeps) \subseteq FinalizedTask
